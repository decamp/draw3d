 ...
 * <p>
 * This kd tree uses a non-trivial Surface-Area-Heuristic for partitioning the
 * space. While the resulting tree is likely to be near optimal, the tree
 * construction is performed using a brute-forcey technique that runs in O(n^2)
 * time. As a result, this tree is not practical for large geometry sets.
 * 
...
public final class NaiveSahKdTree implements RayPicker {
    ...
    public boolean intersectTree( Node node,
                                  double[] rayPoint,
                                  double[] rayDir,
                                  double minDist,
                                  double maxDist,
                                  ...
                                  Result result )
    {
        ...
        if( node.isLeaf() ) {
            ...
            for( Triangle t : node.mTriangles ) {
                ...
                if( intersect.mRayDist >= result.mRayDist || intersect.mRayDist > maxDist + Tol.TOL ||
                    intersect.mRayDist < minDist - Tol.TOL ) {
                    continue;
                }
    ...
    private static Node build( int depth, List<Triangle> tris, double[] voxel ) {
        ...
        final int splitSide; // Sorting preference for triangles laying
                             // precisely on split plane
