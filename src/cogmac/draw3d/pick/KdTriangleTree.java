...
import cogmac.math3d.geom.Loop;
...
/**
 * KdTriangleTree indexes collections of Triangle objects and provides functionality
 * for fast geometry picking using the RayPicker interface.
 * <p>
 * This class is a complete implementation of the KD tree described in: <br/>
 * Wald, Ingo and Havran, Vlastimil. <i>On building fast kd-Trees for Ray Tracing, 
 * and on doing that in O(N log N).</i> <br/>
 * Proceedings of the 2006 IEEE Symposium on Interactive Ray Tracing, 2006, pages 61-69. <br/>
 * <p>
 * Building a tree occurs in O(N log N) time, and tree traversal runs in O(log N).  Although
 * any tree implementation will run traversals in O(log N), in practice, there are significant 
 * differences in the performance.  This implementation employs several algorithms to help improve
 * the space partitioning:
 * <p>
 * A <i>Surface Area Heuristic</i> is used to generate reasonable probabilities for the 
 * likelihood of rays intersecting candidate space partitions.
 * <p>
 * Partition costs are biased towards culling out empty spaces, which, according to Monsieur Ingo Wald,
 * improves traversal speed in most cases.
 * <p>
 * Triangles are perfectly split, which means that for triangles the span partition planes, the
 * triangles are clipped exactly to each partition space, instead of the far far far easier practice 
 * of clipping the <i>bounding box</i> of the triangle to the partition box.  This means that triangles 
 * are never added to a leaf node that they do not actually intersect.
 * 
 * @author decamp
 */
public final class KdTriangleTree implements RayPicker {
    ...
    public static KdTriangleTree build(List<? extends Triangle> triList) {
        ...
        Aabbs.aabbToArray(Triangles.computeBounds(triList), vox);
    ...
    private boolean intersectTree(Node node, double[] rayPoint, double[] rayDir, double minDist, double maxDist, int sides, Result result) {
                               ...
                               double[] voxel) 
    {
        ...
        boolean havePlane = findSplitPlane(depth, triCount, events, 0, eventCount, voxel, plane);
        ...
        final int splitSide   = plane.mSide;    //Sorting preference for triangles laying precisely on split plane
        ...
        for(int i = 0; i < eventCount; i++) {
            ...
            case BOTH:
                //Reclip triangles.
            {
                triMark[e.mTriIndex] = NONE;     //Leave indication that new events have now been generated.
                Triangle t = tris[e.mTriIndex];
                verts[0] = t.vertex(0);
                verts[1] = t.vertex(1);
                verts[2] = t.vertex(2);
                
                if(Aabbs.clipPlanarToAabb(verts, 0, 3, leftVoxel, loop)) {
                    Aabbs.boundPoints(loop.mVerts, 0, loop.mLength, triVoxel);
                    for(int j = 0; j < 3; j++) {
                ...
                if(Aabbs.clipPlanarToAabb(verts, 0, 3, rightVoxel, loop)) {
                    Aabbs.boundPoints(loop.mVerts, 0, loop.mLength, triVoxel);
                    for(int j = 0; j < 3; j++) {
                        if(triVoxel[j] == triVoxel[j+3]) {
    ...
    private static boolean findSplitPlane( int depth, 
                                           int triCount, 
                                           SplitEvent[] events,
                                           int eventOff,
                                           int eventCount,
                                           ...
                                           Plane out) 
    {
        ...
        for(int i = eventOff; i < eventOff + eventCount;) {
            ...
            while(i < eventCount && splitAxis == events[i].mAxis && splitPos == events[i].mPosition && events[i].mType == EVENT_STOP) {
                newStops++;
                i++;
            }
            
            while(i < eventCount && splitAxis == events[i].mAxis && splitPos == events[i].mPosition && events[i].mType == EVENT_PLANAR) {
                newPlanars++;
                i++;
            }
            
            while(i < eventCount && splitAxis == events[i].mAxis && splitPos == events[i].mPosition && events[i].mType == EVENT_START) {
                newStarts++;
                i++;
            }
    ...
    private static <T extends Comparable<? super T>> void sortlessMerge(T[] a, int aStart, int aStop, T[] b, int bStart, int bStop, T[] out, int outOff) {
