...
/**
 * Sorry, no comments yet.  SceneGraph is not thread-safe and not 
 * really dynamically modifiable, yet.
 * 
 * @author decamp
 */
public class SceneGraph {
    ...
    /**
     * Replaces a node with a new object, while retaining all existing connections.
    ...
    private SceneGraph flatten(Stack<SceneGraph> history) {
        ...
        for(NodeInfo mod: mModuleMap.values()) {
            for(GraphEdge edge: mod.mChildEdges) {
                ...
                //Check graph behavior.  This may change in the future, or be a variable.
                if(allList == null) {
    ...
    private List<NodeInfo> findLeaves(Set<NodeInfo> modules) {
        List<NodeInfo> ret = new ArrayList<NodeInfo>();
        
        for(NodeInfo n: modules) {
            if(n.mChildEdges.isEmpty()) {
                ret.add(n);
            }
        }
    ...
    private GraphPath<NodeInfo> compileAllNodePaths() {
        ...
        for(NodeInfo root: roots) {
            //ret.add(new GraphStep<SceneNode>(GraphStep.Action.START_PATH, null));
