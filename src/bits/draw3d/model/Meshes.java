...
public final class Meshes {
    ...
    public static void normalizeNorms( Collection<Triangle> tris ) {
        for( Triangle t: tris ) {
            if( t.mNorms != null ) {
                Vectors.normalize( t.mNorms[0], 1.0 );
                Vectors.normalize( t.mNorms[1], 1.0 );
                Vectors.normalize( t.mNorms[2], 1.0 );
            }
        }
    }
    ...
    public static void setColor( Collection<? extends Triangle> tris, double r, double g, double b, double a ) {
        for( Triangle t : tris ) {
            t.mColors = new double[][]{{ r, g, b, a },
                                       { r, g, b, a },
                                       { r, g, b, a }};
        }
    }
    ...
    public static void assignTexCoords2( List<Triangle> tris, double[] mat4 ) {
        double[] work = new double[3];
        for( Triangle t: tris ) {
            t.mTexs = new double[3][2];
            for( int i = 0; i < 3; i++ ) {
                Matrices.multMatVec( mat4, t.mVerts[i], work );
                t.mTexs[i][0] = work[0];
                t.mTexs[i][1] = work[1];
            }
        }
